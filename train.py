...
def load_dataset() -> [CUDAPrefetcher, CUDAPrefetcher, CUDAPrefetcher]:
    ...
    train_datasets = TrainValidImageDataset(image_dir=config.train_image_dir,
                                            image_size=config.image_size,
                                            upscale_factor=config.upscale_factor,
                                            pca_matrix_path=config.pca_matrix_path,
...
def define_scheduler(optimizer) -> lr_scheduler.CosineAnnealingWarmRestarts:
    scheduler = lr_scheduler.CosineAnnealingWarmRestarts(optimizer,
                                                         T_0=config.lr_scheduler_T_0,
...
def train(model, train_prefetcher, psnr_criterion, pixel_criterion, optimizer, epoch, scaler, writer) -> None:
    ...
    while batch_data is not None:
        ...
        batch_kernel = batch_data["batch_kernel"].to(config.device, non_blocking=True)
