...
import config
import imgproc
from model import SFTMD


def main() -> None:
    # Initialize the super-resolution model
    model = SFTMD(config.upscale_factor).to(config.device)
    print("Build SFTMD model successfully.")

    # Load the super-resolution model weights
    checkpoint = torch.load(config.model_path, map_location=lambda storage, loc: storage)
    model.load_state_dict(checkpoint["state_dict"])
    print(f"Load SFTMD model weights `{os.path.abspath(config.model_path)}` successfully.")
    ...
    for index in range(total_files):
        ...
        batch_kernel_process = imgproc.SRMDPreprocessing(upscale_factor=config.upscale_factor, pca_matrix_path=config.pca_matrix_path, random=True,
                                                         kernel_size=config.kernel_size, noise=False, device=config.device, is_tensor=True,
                                                         sigma=config.sigma, min_sigma=config.min_sigma, max_sigma=config.max_sigma, iso_prob=1.0,
                                                         scaling=3, noise_coeff=0.2, noise_high=0.0)

        batch_kernel = batch_kernel_process(hr_tensor)

        # Only reconstruct the Y channel image data.
        with torch.no_grad():
            sr_tensor = model(lr_tensor, batch_kernel)
