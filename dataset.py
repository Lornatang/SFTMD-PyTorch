...
class TrainValidImageDataset(Dataset):
    ...
    Args:
        ...
        pca_matrix_path (str): PCA matrix address.
        kernel_size (int): Gaussian kernel size.
        sigma (float): Gaussian blur sigma value.
        min_sigma (float): Gaussian blur min sigma value.
        max_sigma (float): Gaussian blur max sigma value.
        device (torch.device): PyTorch device type.
    ...
    def __init__(self, image_dir: str, image_size: int, upscale_factor: int, pca_matrix_path: str, kernel_size: int, sigma: float, min_sigma: float,
                 max_sigma: float, device: torch.device, mode: str) -> None:
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, torch.Tensor]:
        ...
        batch_kernel = self.batch_kernel_process(hr_tensor)

        return {"lr": lr_tensor, "hr": hr_tensor, "batch_kernel": batch_kernel}
...
class TestImageDataset(Dataset):
    ...
    def __init__(self, test_lr_image_dir: str, test_hr_image_dir: str, upscale_factor: int, pca_matrix_path: str,
                 kernel_size: int, sigma: float, min_sigma: float, max_sigma: float, device: torch.device) -> None:
